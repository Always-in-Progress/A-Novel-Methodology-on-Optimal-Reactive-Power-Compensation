%% Grey Wolf Equilibrium Optimizer for Optimal Reactive Power Compensation
...
function [Alpha_score, Alpha_pos, Convergence_curve, Q_C, Q_proportion_plot, ...
    ...
    while iter <= Max_iter
        ...
        t = (1 - iter / Max_iter) ^ (2 * a2 * iter / Max_iter);                             % Eq (9)
        a = 2 .* t;
        ...
        for i = 1:size(Positions,1)
            ...
            G = G0 .* F;                                              % Eq(13)
            Position_Alpha = Ceq + ((Positions(i, :) - Ceq) .* F + (G ./ lambda * V) .* (1 - F)) - A.*abs(D);                            % Eq(16) for ALPHA
            ...
            G0 = GCP .* (Ceq - lambda .* Positions(i, :));        % Eq(14)
            G = G0 .* F;                                              % Eq(13)
            Position_Beta = Ceq + ((Positions(i, :) - Ceq) .* F + (G ./ lambda * V) .* (1 - F)) - A .* abs(D);
            % Eq(16) for BETA
            ...
            G0 = GCP .* (Ceq - lambda .* Positions(i, :));        % Eq(14)
            G = G0 .* F;                                              % Eq(13)
            Position_Delta = Ceq + ((Positions(i, :) - Ceq) .* F + (G ./ lambda * V) .* (1 - F)) - A .* abs(D); 
            % Global
