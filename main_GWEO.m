...
fprintf('Grey wolf equilibrium optimiser (GWEO) is implemented to the problem.\n');
...
C_bank_proportion_lb = zeros(1, 4);
C_bank_proportion_ub = ones(1, 4);
L_bank_proportion_lb = zeros(1, 4);
L_bank_proportion_ub = ones(1, 4);
lb = [Cap_lb, C_bank_proportion_lb, L_bank_proportion_lb];
ub = [Cap_ub, C_bank_proportion_ub, L_bank_proportion_ub];
Mode = [1, 1];

%% Optimisation
% Determine inherent parameters in the employed GWEO
SearchAgents_no = 50;
Max_iter = 200;
dim = length(lb);
% Realise optimal reactive power compensation by employing GWEO
[E_Q, Alpha_pos, Convergence_curve, Q_C, Q_proportion_plot, ...
      schedule_info_Cap, C_bank_status_info, ...
      schedule_info_Rea, L_bank_status_info] = GWEO_for_ORPC(SearchAgents_no, Max_iter, lb, ub, dim, ...
                                                             T_sample, Q_L, Max_switch_times, Mode);
Cap_capacitors = Alpha_pos(:, 1);
Cap_reactors = Alpha_pos(:, 2);
C_bank_proportion = Alpha_pos(:, 3:6);
L_bank_proportion = Alpha_pos(:, 7:10);
delta_Q = Q_L - Q_C;
